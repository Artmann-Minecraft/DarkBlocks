		...
		MySQLUtils.get(mySQL, "Userdata", "*", "uuid", uniqueId, result ->
		{
			try
			{
				if (result.next())
				{
					...
					User user = new User(groups, new HashSet<>(), uniqueId, result.getString("prefix"), result.getString("suffix"), lowestSortID);
				...
				else
				{
					...
					mySQL.update("INSERT INTO Userdata(`uuid`, `groups`, `firstonline`, `lastonline`, `prefix`, `suffix`) VALUES ('" + uniqueId + "','" + singleton + "','" + date + "','" + date + "','" + defaultGroup.getPrefix() + "','" + defaultGroup.getSuffix() + "'");
				}
			} catch (SQLException ex)
			{
				ex.printStackTrace();
